...
def load_and_process_data():
    ...
    try:
        # Try to load the data file (adjust filename as needed)
        df = pd.read_csv('FMEL_Dataset.csv')  # Update this to your actual filename
        ...
        # Convert date to datetime and extract features (handle DD/MM/YYYY format)
        try:
        ...
        return df, le_local, le_visitor, le_season, le_division, le_round
        
    except Exception as e:
        st.error(f"Error loading data: {str(e)}")
        st.error("Please make sure your CSV file is in the repository and named correctly!")
...
def main():
    ...
    with st.spinner("Loading and processing data..."):
        df, local_encoder, visitor_encoder, season_encoder, division_encoder, round_encoder = load_and_process_data()
    ...
    with st.spinner("Training prediction model..."):
        model, accuracy = train_model(df.copy(), team_stats)
